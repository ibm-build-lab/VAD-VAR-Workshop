---
title: 'Usecase: Smart contract processing'
timeToComplete: 30
updated: 2024-05-24
---
...
A property management contract between the owner and a firm contains various clauses. The PDF of the legal contract document is converted to text. The PII in the text is masked. The masked document is processed for detecting important missing information, essential to the format, e.g. property address, and mandatory legal clauses, e.g. renewal terms, for completeness of the document.

## Step 2. Try the interactive demo application

- User uploads existing sample contracts in PDF format. The document is available to view from the links in the app. One of them is missing critical clauses required for approval. The goal is to check if these can be detected using a prompt engineered LLM.

- Watson Document Understanding (internal release) is used to extract HTML text from the document and displayed in the left pane.

- The HTML text is then processed through a watsonx.ai LLM to detect PII, e.g. date, name, amount, address.
- The PII entities are replaced with generic information and the generic document is then processed through a watsonx.ai LLM to detect missing critical information and legal clauses.
- A recommendation is generated for approval or addition of key missing clauses.
- The actual API call and models used can be seen in 'Settings ⭢ View payload'.

![alt text](images/image1.png)

[Let's try this interactive live demo](https://94d58df60b.dsceapp.buildlab.cloud/)
