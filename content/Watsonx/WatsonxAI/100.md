...
1. To generate an API key from your IBM Cloud user account, go to [Manage access and users - API Keys](https://cloud.ibm.com/iam/apikeys) and click on **Create**. In the pop-up, provide a name like `watsonx-apikey` and a short description, then click **Create**

   ![create api key](/images/image-004.png)

1. Immediately make a copy of the API key and store it in a secure vault (like 1Password, etc.) as there will be no way to retrieve it again.
...
1. From the list, select the project that your are using for the integration.

1. Click on the **Manage** tab and check that **General** is selected. Copy the **Project ID** to a location where you can access it later. This is not sensitive like the API key so it can be in notepad or something equivalent.
...
1. Scroll down to the **Extensions** section and click on the **Build custom extension** button:

   ![build custom extension](/images/image-001.png)

1. Review the getting started guide and click on **Next**

1. On the basic information tab, name the extension `watsonx` and provide an optional description. Click **Next**

1. Either drag and drop or use the file browser to select the `watsonx-openapi.json` file and then click **Next**

   ![load watsonx-openapi.json](/images/image-002.png)

1. Review the information displayed and click on **Finish**, now the integration catalog shows the integration

   ![watsonx integration in catalog](/images/image-003.png)

1. Click on **Add** in the lower right corner of the new integration tile, confirm **Add** when prompted

1. Review the Get started information and click on **Next**

1. In the Authentication panel, select `Oauth 2.0`, enter in the API key that you created earlier, keep the remaining default values and then click **Next**

   ![set authentication](/images/image-008.png)

1. On the last page review the `POST` request parameters and response values. The assistant can set parameters in the request before calling the extension and will parse the response to show the user the output. Click on **Finish**, then click on **Close** if you are not returned to the **Extensions** section of the **Integrations** panel.
...
1. In the main panel, click on **Create action**

1. Select **Start from Scratch**

1. For what the customer says enter `Marketing Message` and click on **Save**

   ![create intent](/images/image-005.png)

1. Click on the pencil icon to set a step title. Use `Prompt for a prompt` as the title.

1. For what the assitant says, you can either make it a short message or be more prescriptive. Note - humans, like generative AI models can often do better when requests are specific. For example:
   ...
   ![define step 1](/images/image-006.png)
...
1. Name the next step `Call watsonx.ai endpoint` by clicking on the pencil (edit) icon. In the **And then** section, select **Use an extension**

    ![use extension](/images/image-007.png)

1. For the extension to use, select `watsonx`. Then select the `Generation` operation to show the parameters that can be sent to watsonx.ai on every invocation. For version, choose `Enter text` and then provide `2023-05-29`, For input, choose `Action Step Variables` and then choose `1. Prompt for a prompt`, continuing in the same way, set the model_id to `google/flan-t5-xxl` and the project_id to your watsonx.ai project id saved before. Don't apply yet

   ![required model params](/images/image-009.png)

1. Expand the optional parameters. For this text generation scenario, use temperature `0.8`, set max_net tokens to `200`, set min_new_tokens to `50` and set repetition penalty to `2`, set decoding_method to `sample`. Then click on **Apply**

   ![required model params](/images/image-010.png)

1. Click on **New Step**

1. In this step, select **with conditions** . In the condition, click on the first item and select **watsonx (step 2)**

   ![set condition](/images/image-011.png)

1. Select **Ran successfully**

1. Click on **Set variable values** , then click on **Set new value**, then click on **New session variable**

   ![add session variable](/images/image-012.png)

1. Call the variable `result` and select `free text` and then click on **Apply**

   ![result variable](/images/image-013.png)

1. For the variable assignment **To**, select **Expression** and type `$` after this is pasted a pulldown will appear. Select `watsonx (step 2)`

   ![result variable](/images/image-014.png)
   ...
   ![extract generated_text](/images/image-014b.png)

   click **Apply**

1. The full expression for the variable should be shown as:

   ![result variable assignment](/images/image-015.png)

1. In the **Assistant says** panel, select **function** and then type in `${result}`, or you can select **Session variables** and then `result`

   ![assistant replies with result](/images/image-016.png)
...
1. After the greeting, type `I need to create a marketing message`, and send to the chatbot.

1. The chatbot will reply with the prompt for the prompt.

1. Enter the following text into the chatbot
...
1. Send to the chatbot - if all worked, you will get a response back from the model with the message.

   ![working assistant](/images/image-017.png)
